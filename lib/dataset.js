...
class Dataset {
  ...
  stream(pageSize = 50, startPage = 0) {
    ...
    return miss.from.obj(function(_size, next) {
      ...
      that.list(pageSize, currentPage)
      .then((datasetList) => {
        currentPage += 1
  ...
  //   stream.drainBufferUntilBackPressure = function() {
  //     while(recordBuffer.length > 0 && !backPressure) {
  //       backPressure = !this.push(recordBuffer.shift())
  //     }
  //   }
  ...
  //         const endOfRows = datasetList.length < pageSize
  //         if(endOfRows) {
  //           recordBuffer.push(null)
  //         }

  //         this.drainBufferUntilBackPressure()
  //       })
  //       .catch((err) => {
  //         this.emit('error', err)
  //       })
